...
based on code from: https://github.com/nicholasdavies/newcovid/blob/master/multinomial_logistic_fits/multinomial%20logistic%20fits_FINAL.R
...
mlr_cladeprops_usa <- multinom.collapse(cladeprops_usa) %>%
...
#Plot clade, no spline
```{r}

mlr_clade_df_1 <- multinom.collapse(df = cladenum %>% filter(location == "USA"),
                                         kthresh = .15,
                                         coldate = date,
                                         colname = variant,
                                         colcount = sequences,
...
#Park, 2021, when the true ratio of Rvoc/Rwt is fixed, then epidemiological conditions describe growth rates, rationale for time varying trends (splines)
...
```
#Make function for growth rate
```{r}

library(emmeans)

trend_normal <- emtrends(mod1, ~ variant | time, var = "time", at = datetime )
summary(trend_normal)

library(ggplot2)

# Convert to data frame for plotting
trend_df <- as.data.frame(summary(trend_normal)) %>% left_join(datetime)

library(ggplot2)
library(dplyr)

ggplot() +
  # Growth rate trend with ribbon for confidence interval
  geom_line(data = trend_df %>% filter(variant == "21L"),
            aes(x = date, y = time.trend, color = "Growth Rate"), size = 1) +
  geom_ribbon(data = trend_df %>% filter(variant == "21L"),
              aes(x = date, ymin = lower.CL, ymax = upper.CL, fill = "Growth Rate"), alpha = 0.2) +
  
  # Overlay predicted frequency
  geom_line(data = pred1_pred %>% filter(variant == "21L"),
            aes(x = date, y = mean, color = "Predicted Proportion"), linetype = "dashed") +

  labs(title = "Growth Rate and Predicted Proportion Over Time for Variant 21L",
       y = "Value", x = "Date", color = "Legend", fill = "Legend") +
  scale_color_manual(values = c("Growth Rate" = "steelblue", "Predicted Proportion" = "darkred")) +
  scale_fill_manual(values = c("Growth Rate" = "steelblue")) +
  theme_minimal()
...
# Grid of time with spline basis


# emtrends for each variant at each spline-based time
model_spline <- emtrends(mod1_spline, ~ variant | s1 , var = "s1", at = splinetime)



trend_spline <- emtrends(mod1_spline, ~ variant, var = c("s1"))
summary(trend_spline)
...
t <- multinom.collapse(df = pangonum %>% filter(location == "USA"), coldate = date, colname = variant, colcount = sequences) 
