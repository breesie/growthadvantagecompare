...
mlr_cladeprops_usa <- cladeprops_usa %>% (date >= mindate & date <= maxdate) %>% 
...
# Make sure 'variant' is treated as a factor
df$variant <- as.factor(df$variant)

# Plot
ggplot(pred1$plotdata, aes(x = time, y = mean, color = variant, fill = variant)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2, color = NA) +
  labs(
    title = "Variant Proportions Over Time with Confidence Intervals",
    x = "Time",
    y = "Proportion"
  ) +
  theme_minimal() +
