...
#New max ad min date, filter to where first emerged pango prop above 30% and add a year 
...
t <- pangoprops %>% filter(variant == "BA.1" & pangoprop > .30) 
...
ggplot(data = cladeprops_usa %>% filter(variant == c("21K", "21L", "22A", "22B"), date > mindate & date < maxdate), aes(x = date, y = cladeprop, color = variant)) +
...
ggplot(data = pangoprops_usa %>% filter(variant == c("BA.1", "BA.2", "BA.4", "BA.5"), date > mindate & date < maxdate), aes(x = date, y = pangoprop, color = variant)) +
  geom_point(alpha = 0.7) +
  labs(
